...
A number, always the log of the magnetic field
...
The tangled field case is the simplest and has two further options.
The first number specifies the magnetic field in one of two ways.
If the keyword \cdCommand{beta} appears then the field is specified in
terms of a $\beta$ parameter related to the ratio of gas to magnetic field pressures.
In much of astronomy, $\beta$ is defined as
\begin{equation}
\beta \equiv \frac{P_{gas}}{P_{magnetic}}
\end{equation}
so a gas-pressure dominated cloud will have a very large $\beta$.
Plasma physics defines $\beta$ as the inverse, so a strongly magnetized
region has large $\beta$.
The optional second number is the index for the gamma-law relation between 
the magnetic field and the gas density.
